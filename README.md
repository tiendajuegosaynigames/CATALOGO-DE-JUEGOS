...
<head>
    ...
    <title>GameBoard - Tienda de Juegos de Mesa</title>
    <style>
        * {
            margin: 0;
            padding: 0;
...
<body>
    <header>
        <nav>
            <div class="logo">🎲 GameBoard</div>
            <div class="search-bar">
                <input type="text" id="searchInput" placeholder="Buscar juegos..." onkeyup="searchProducts()">
            </div>
            <button class="cart-btn" onclick="showCart()">🛒 Carrito (<span id="cartCount">0</span>)</button>
        </nav>
    </header>
    ...
    <footer>
        <p>&copy; 2024 GameBoard - Tienda de Juegos de Mesa | Todos los derechos reservados</p>
        <p>📧 contacto@gameboard.com | 📞 +591 123-4567</p>
    </footer>
    ...
    <script>
            ...
            container.innerHTML = filteredProducts.map((product, index) => `
                <div class="product-card">
                    <div class="product-img" style="background: ${gradients[index % gradients.length]}">
                        ...
                        ${product.name}
                        <div style="position: absolute; bottom: 10px; right: 10px; background: rgba(0,0,0,0.5); padding: 5px 10px; border-radius: 10px; font-size: 0.8rem;">
                            📷 Agregar imagen
                        </div>
                    </div>
                    <div class="product-info">
                        ...
                        <div class="product-footer">
                            ...
                            <button class="buy-btn" onclick="addToCart('${product.name}', ${product.price})">Comprar</button>
                        </div>
                    </div>
                </div>
            `).join('');
        ...
        function showCart() {
            ...
            const items = cart.map(item => `• ${item.name}: Bs ${item.price}`).join('\n');
            alert(`🛒 Tu Carrito:\n\n${items}\n\n💰 TOTAL: Bs ${total}`);
        }
